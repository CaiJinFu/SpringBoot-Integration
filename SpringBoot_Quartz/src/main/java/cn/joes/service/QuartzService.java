...
import cn.joes.job.impl.HelloJob;
import org.quartz.*;
...
public class QuartzService {
    ...
    public void addJob(String jobClassName, String jobName, String groupName, ScheduleBuilder scheduleBuilder, Map<String, Object> jobDataMap) throws Exception {
       /* if (!isBaseJob(jobClassName)) {
        ...
        //构建job信息
        JobDetail jobDetail = JobBuilder.newJob(getClass("cn.joes.job.impl." + jobClassName).getClass()).withIdentity(jobName, groupName).build();
    ...
    public static Boolean isBaseJob(String className) {
        try {
            Class<?> jobClass = Class.forName(className);
            if (jobClass.isInstance(BaseJob.class)) {
                return true;
            }
